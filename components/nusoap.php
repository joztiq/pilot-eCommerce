...
/*
$Id: nusoap.php,v 1.2 2011-03-22 11:27:11 maidan Exp $
...
* @version  $Id: nusoap.php,v 1.2 2011-03-22 11:27:11 maidan Exp $
* @access   public
*/
class nusoap_base {
	/**
...
class wsdl extends nusoap_base {
	...
	*/
	function serialize($debug = 0)
	{
		...
		$xml .= '<?xml-stylesheet type="text/xsl" href="'.Joztiq::app()->config->rootPath.'/public/resources/wsdl-viewer.xsl"?>';
		$xml .= "\n<definitions";
		foreach($this->namespaces as $k => $v) {
			$xml .= " xmlns:$k=\"$v\"";
		} 
			...
			if (is_array($value) && sizeof($value) >= 1) {
				...
				foreach($value as $k => $v) {
					if(!is_object($v))
					{
						...
						$this->debug("serializing array element: $k, ".get_class($v)." of type: $typeDef[arrayType]");
					}
...
//					if (strpos($typeDef['arrayType'], ':') ) {
					if (!in_array($typeDef['arrayType'],$this->typemap['http://www.w3.org/2001/XMLSchema'])) {
						if(is_object($v))
						{
							$contents .= $this->serializeType(get_class($v), $typeDef['arrayType'], $v, $use);
						}else{
